...
/*                {
                    ...
                    text: formatMessage({
                        ...
                        default: 'when input [GPIO] is [ACTIVE]',
                        description: 'when the input pin is active'
                    }),
                ...
                },
                {
                    ...
                    text: formatMessage({
                        id: 'pigpio.is_active',
                        default: 'input [GPIO] is [ACTIVE] ?',
                        description: 'is the input pin active?'
                    }),
                ...
                },
                {
                    ...
                    text: formatMessage({
                        id: 'pigpio.set_input',
                        default: 'set input [GPIO] to [PULL]',
                        description: 'set gpio as input'
                    }),
                ...
                },
                {
                    opcode: 'set_output',
                    text: formatMessage({
                        id: 'pigpio.set_output',
                        default: 'set output [GPIO] [STATE]',
                ...
                {
                    opcode: 'toggle_output',
                    text: formatMessage({
                        id: 'pigpio.toggle_output',
                        default: 'toggle output [GPIO]',
                        description: 'set gpio to value'
                    }),
                    blockType: BlockType.COMMAND,
                    arguments: {
                ...
                },
                {
                    opcode: 'get_output',
                    text: formatMessage({
                        id: 'pigpio.get_output',
                        default: 'output [GPIO] is [ONOFF] ?',
                        description: 'is the output pin on?'
                    }),
                    blockType: BlockType.BOOLEAN,
                    arguments: {
                        GPIO: {
    ...
    {
        const pin = Cast.toString (args.GPIO);
        const state = Cast.toString (args.PULL);

        let op = 'pu';
        if (state == 'pull down') op = 'pd';
    ...
    {
        const pin = Cast.toString (args.GPIO);
        const active = Cast.toString (args.ACTIVE);

        let act = true;
        if (active === 'inactive') act = false;

        const command = 'raspi-gpio get ' + pin
        const result = cp.execSync (command).toString();
        const elements = result.split (' ');

        if (elements[5].includes ('DOWN'))
        {
            if (elements[2].includes ('1')) return act;
            else return !act;
        }
        if (elements[5].includes ('UP'))
        {
    ...
    {
        const pin = Cast.toString (args.GPIO);
        const active = Cast.toString (args.ONOFF);

        let act = true;
        if (active === 'off') act = false;
